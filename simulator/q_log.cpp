...
QLog::QLog(uint64_t log_capacity, stats::LocalStatsCollector &log_stats)
    : _log_stats(log_stats),
      _index(bitCalc(log_capacity)),
      _total_capacity(log_capacity),
      _total_size(0) {}

std::vector<candidate_t> QLog::insert(std::vector<candidate_t> items) {
...
bool QLog::find(candidate_t item) {
  if (_index.find(item)) {
    _log_stats["hits"]++;
    return true;
  } else {
...
void QLog::readmit(std::vector<candidate_t> items) {
  for (auto &item : items) {
...
void QLog::insertFromSets(candidate_t item) {
  if (_index.readmit(item)) {
    _total_size += item.obj_size;
  }
  _log_stats["current_size"] = _total_size;
