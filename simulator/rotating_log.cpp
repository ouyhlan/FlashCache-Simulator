...
RotatingLog::RotatingLog(uint64_t log_capacity, uint64_t block_size,
                         SetsAbstract* sets,
...
std::vector<candidate_t> RotatingLog::_addSetMatches(
    std::vector<candidate_t> evicted) {
  if (_sets == nullptr) {
    return evicted;
  }
  ...
  for (auto item : evicted) {
    ...
    if (_set_to_items[set_num].size()) {
      ...
      for (auto item_evicted : _set_to_items[set_num]) {
        bool already_evicted =
            _item_active.find(item_evicted) == _item_active.end();
...
void RotatingLog::readmit(std::vector<candidate_t> items) {
  for (auto item : items) {
    ...
    if (_item_active.find(item) != _item_active.end()) {
      ...
      if (_blocks[_active_block]._size + item.obj_size >
          _blocks[_active_block]._capacity) {
...
double RotatingLog::calcWriteAmp() {
  double ret = _log_stats["bytes_written"] /
