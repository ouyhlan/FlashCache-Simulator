...
MemLogSetsCache::MemLogSetsCache(stats::StatsCollector* sc,
  ...
  uint64_t actual_set_capacity =
      exact_set_capacity - std::fmod(exact_set_capacity, set_capacity);
  ...
  if (cfg.exists("sets.rripBits")) {
    ...
    _sets = new flashCache::RripSets(num_sets, set_capacity, set_stats, this,
                                     num_hash_functions, rrip_bits, promotion,
                                     mixed);
  } else {
    bool sets_track_hits = cfg.exists("sets.trackHitsPerItem");
    _sets = new flashCache::Sets(num_sets, set_capacity, set_stats, this,
                                 num_hash_functions, sets_track_hits);
  }
  ...
  if (cfg.exists("log.flushBlockSizeKB")) {
    ...
    uint64_t block_size =
        1024 * (uint64_t)cfg.read<int>("log.flushBlockSizeKB");
    _log = new flashCache::RotatingLog(log_capacity, block_size, _sets,
                                       log_stats, readmit);
  } else {
    auto& log_stats = statsCollector->createLocalCollector("log");
  ...
  double perc_mem_log_overhead =
      cfg.read<float>("cache.memOverheadRatio", INDEX_LOG_RATIO);
  assert(log_capacity * perc_mem_log_overhead + sets_memory_consumption <=
         memory_size);
  uint64_t mem_cache_capacity =
      memory_size - (log_capacity * perc_mem_log_overhead);
  ...
  _memCache = new memcache::S3LRU(mem_cache_capacity, memory_cache_stats);
  ...
  if (cfg.exists("preSetAdmission")) {
    ...
    const libconfig::Setting& admission_settings =
        cfg.read<libconfig::Setting&>("preSetAdmission");
    auto& admission_stats = statsCollector->createLocalCollector(policyType);
    _preset_admission = admission::Policy::create(admission_settings, _sets,
                                                  _log, admission_stats);
  }
  ...
  if (ret.size()) {
    if (warmed_up && _preset_admission) {
      ...
      for (auto set : admitted) {
        if (_record_dist) {
          std::string num_name =
              "numItemsMoved" + std::to_string(set.second.size());
          uint64_t size =
              std::accumulate(set.second.begin(), set.second.end(), 0,
