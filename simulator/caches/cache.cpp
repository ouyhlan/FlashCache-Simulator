...
Cache::Cache(stats::StatsCollector* sc, stats::LocalStatsCollector& gs,
             const libconfig::Setting& settings)
    : statsCollector(sc), globalStats(gs), _historyAccess(false) {
  misc::ConfigReader cfg(settings);
  int stats_power =
      cfg.read<int>("stats.collectionIntervalPower", STATS_INTERVAL_POWER);
  _stats_interval = pow(10, stats_power);
}

Cache::~Cache() { delete statsCollector; }
...
Cache* Cache::create(const libconfig::Setting& settings) {
  ...
  bool qlog = cfg.exists("qlog");

  if (memoryCache && !log && sets && !qlog) {
    cache_ret = new SetOnlyCache(sc, gs, settings);
  } else if (memoryCache && log && sets && !qlog) {
    cache_ret = new MemLogSetsCache(sc, gs, settings);
  } else if (memoryCache && log && sets && qlog) {
    cache_ret = new ZoneCache(sc, gs, settings);
  } else if (memoryCache && log && !sets && !qlog) {
...
void Cache::dumpStats() {
  ...
  if (globalStats["memHits"] > 0) {
    double memHitPer = globalStats["memHits"] / (double)globalStats["hits"];
    double logHitPer = globalStats["logHits"] / (double)globalStats["hits"];
    double setHitPer = globalStats["setHits"] / (double)globalStats["hits"];
    std::cout << "Mem : Log : Set Hit Percentage: " << memHitPer << " : "
              << logHitPer << " : " << setHitPer << std::endl;
  }

  statsCollector->print();
}

uint64_t Cache::getTotalAccesses() { return globalStats["totalAccesses"]; }
